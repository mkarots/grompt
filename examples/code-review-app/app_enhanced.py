...
class CodeReviewer:
    ...
    def review(
        self, code: str, language: str = "Python", examples: Optional[List[str]] = None
    ) -> dict:
        """
        ...
        rendered = prompt.render(code=code, language=language, examples=examples or [])
...
Review: Excellent use of parameterized queries to prevent SQL injection. Well done!""",
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        result = reviewer.review(
            code=test["code"], language=test["language"], examples=test.get("examples")
        )
